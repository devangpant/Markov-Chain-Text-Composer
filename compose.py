...
Overall Workflow:
1. Read a text file.
2. Preprocess(lowercase all text, remove punctuation, remove tab space, remove new para space) the text and obtain a list of words.
3. Construct a Markov Chain graph based on word occurrences. (connect nodes, add weight to edges)
4. Generate a new text composition using the Markov Chain model.
...
def make_graph(words):
    ...
    for word in words:
        ...
        # if there was a previous word, then add an edge if it does not exist
